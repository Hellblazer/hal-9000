#!/usr/bin/env bash
# cs-list.sh - List all active ClaudeBox Squad sessions
...
sessions=$(tmux list-sessions 2>/dev/null | grep "^squad-" || true)

if [[ -z "$sessions" ]]; then
    printf "${YELLOW}No active squad sessions found.${NC}\n\n"
    printf "Run './claudebox-squad.sh' to start sessions\n"
    exit 0
fi
...
printf "\n${CYAN}Commands:${NC}\n"
printf "  ./cs-attach.sh <session-name>  - Attach to a session\n"
printf "  ./cs-stop.sh <session-name>    - Stop a specific session\n"
printf "  ./cs-cleanup.sh                - Stop all sessions and cleanup\n"
printf "\n"
