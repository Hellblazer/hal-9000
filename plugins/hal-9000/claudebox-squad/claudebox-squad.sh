#!/usr/bin/env bash
# claudebox-squad.sh - Manage multiple ClaudeBox instances with git worktrees
# 
# Integrates ClaudeBox with claude-squad-style workflow:
...
#
# Usage: ./claudebox-squad.sh [config_file]

set -Eeuo pipefail
IFS=$'\n\t'

readonly SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
readonly SQUAD_DIR="$HOME/.claudebox-squad"
readonly LOCKFILE="$SQUAD_DIR/squad.lock"
...
init_squad_dir() {
    if [[ ! -d "$SQUAD_DIR" ]]; then
        mkdir -p "$SQUAD_DIR"
        info "Initialized squad directory: $SQUAD_DIR"
    fi
}
...
EOF
    ...
    printf '%s\n' "$state_entry" >> "$SQUAD_DIR/sessions.log"
}

# Main execution
main() {
    local config_file="${1:-squad.conf}"
    ...
    while IFS=: read -r branch profile description; do
        ...
        local session_name="squad-${branch//\//-}"

        # Generate worktree directory
        local worktree_dir="$HOME/.claudebox-squad/worktrees/${repo_name}-${branch//\//-}"
    ...
    info "Use './cs-list.sh' to see active sessions"
    info "Use './cs-attach.sh <session-name>' to attach to a session"
    info "Use './cs-cleanup.sh' to stop all sessions and cleanup"
}
