#!/usr/bin/env bash
# cs-cleanup.sh - Cleanup all ClaudeBox Squad sessions and worktrees
#
...
done < <(tmux list-sessions 2>/dev/null | grep "^squad-" || true)
...
if [[ ! -d "$SQUAD_DIR/worktrees" ]]; then
    ...
    done < <(find "$SQUAD_DIR/worktrees" -mindepth 1 -maxdepth 1 -type d 2>/dev/null || true)
    ...
    if [[ -d "$SQUAD_DIR/worktrees" ]]; then
        rmdir "$SQUAD_DIR/worktrees" 2>/dev/null || true
    fi
fi
...
if [[ -f "$SQUAD_DIR/sessions.log" ]]; then
    rm -f "$SQUAD_DIR/sessions.log"
    success "Removed sessions log"
fi

if [[ -d "$SQUAD_DIR" ]]; then
    # Remove if empty
    rmdir "$SQUAD_DIR" 2>/dev/null && success "Removed squad directory" || true
fi
...
info "You can run './claudebox-squad.sh' to start new sessions"
printf "\n"
